...
func newModel(db *gorm.DB, opts ...gen.DOOption) model {
	...
	_model.Slug = field.NewString(tableName, "slug")
	_model.Details = field.NewString(tableName, "details")
	_model.MakeID = field.NewInt32(tableName, "make_id")
	_model.Created = field.NewTime(tableName, "created")
	_model.Active = field.NewInt32(tableName, "active")
...
func (m *model) updateTableName(table string) *model {
	...
	m.MakeID = field.NewInt32(table, "make_id")
...
type IModelDo interface {
	...
	Unscoped() IModelDo
	Create(values ...*model.Model) error
	CreateInBatches(values []*model.Model, batchSize int) error
	Save(values ...*model.Model) error
	First() (*model.Model, error)
	Take() (*model.Model, error)
	Last() (*model.Model, error)
	Find() ([]*model.Model, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*model.Model, err error)
	FindInBatches(result *[]*model.Model, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*model.Model) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*model.Model, error)
	FirstOrCreate() (*model.Model, error)
	FindByPage(offset int, limit int) (result []*model.Model, count int64, err error)
